...
if 'user_games' not in globals():
    user_games = {}

# ---- Вспомогательная функция для повторной игры ----
async def repeat_game(call, state, new_bet=None):
    ...
    if bet < 0.1:
        await call.answer("❌ Ставка меньше 0.1$", show_alert=True)
        return False
    balance = await get_balance(user_id)
    if bet > balance:
    ...
    if game_type == 'cube':
        ...
        if not mode:
            await call.answer("Ошибка режима", show_alert=True)
            return False
...
# ---- Кубик: обработка выбора (less/more, even/odd, 3.5) ----
...
async def cube_choice_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    mode = user_games[user_id]['mode']
    choice = callback.data.split(':')[1]  # less, more, even, odd, gt35, lt35
...
# ---- Кубик: режим exact (угадать число) ----
